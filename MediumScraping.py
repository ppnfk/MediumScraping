...
def get_img_filename(article):
    try:
        ...
        pass
def create_aem_article(article_link):
    r = requests.get(article_link)
    s = BeautifulSoup(r.text, 'html.parser')
    print(article_link)
    print(s.article.findAll('img')[3].get('src', None))
...
def download_article_images(article):
    ...
    try:
        ...
        path = urllib.parse.urlparse(image_url).path
        ext = os.path.splitext(path)[1]
        filename = article["img_filename"] + ext
        # print(filename)
...
def create_aem_article(article):
    ...
    try:
        ...
        if not article["img_filename"]:
            ...
            aeq_json_template["properties"]["elements"]["image"]["value"] = "/content/dam/wknd/en/articles-assets/" + article["img_filename"]
    ...
    response = requests.post(url+str(article["articleId"]), data=json.dumps(aeq_json_template), headers=newHeaders, auth=('******','******'))
